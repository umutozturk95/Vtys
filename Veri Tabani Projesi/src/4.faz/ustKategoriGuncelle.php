       ...
       if($c2 = OCILogon("C##b21328394", "21328394", $db2))
          {
		    ...
		    $array=oci_parse($c2,"update blog_topcategory set category_name='$title' where topcategory_id =".$_GET['kategori_id']);		
            oci_execute($array);
				
		   
			$array=oci_parse($c2,"insert into logUser values(DEFAULT,'$person_id','Blog ust kategori guncelliyor.',to_date('$delivDate','dd-mm-yy hh24:mi:ss'))");		
            oci_execute($array); 
		 }  
	  oci_close($c2);
	  header("Location:ustKategoriler.php");
   ...
   <body  style="background-color:powderblue; !important;">  
    ...
    <div id="menu">
       ...
       <?php			
		    ...
		    $array3=oci_parse($c,"select topcategory_id,category_name from blog_topcategory where topcategory_id=".$_GET['kategori_id']);
	         oci_execute($array3);
       ...
       ?>  
			...
			<form method="post">
	         	<table>
	              <tr>
		           <td><span style="font-weight:bold;">Üst Kategori İsim:</span></td>
