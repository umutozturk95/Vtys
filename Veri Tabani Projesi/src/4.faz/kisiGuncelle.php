...
if(isset($_POST['guncelle'])){
	  ...
	  $db2 = "(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST= dbs.cs.hacettepe.edu.tr)(PORT=1521))(CONNECT_DATA=(SID=dbs)))";
       if($c2 = OCILogon("C##b21328394", "21328394", $db2))
          {
		    $array2=oci_parse($c2,"update people set name='$name' ,phone_number='$phone_number',email='$email',password ='$encrypted_password'  where person_id=".$_GET['person_id']);
            oci_execute($array2);
			
		
            $array=oci_parse($c2,"insert into logUser values(DEFAULT,'$person_id','Kullanici guncelliyor.',to_date('$delivDate','dd-mm-yy hh24:mi:ss'))");		
            oci_execute($array); 
	   ...
	   header("Location:kisiGoruntule.php?person_id=".$_GET['person_id']);
 }
    ...
    <div id="menu">
      ...
      <?php
           if(isset($_GET['person_id'])){
		         ...
		         $array=oci_parse($c,"select name ,phone_number, email, password,type from people where person_id =".$_GET['person_id']);
                 oci_execute($array);
			     $row=oci_fetch_array($array);
      ?>	  
