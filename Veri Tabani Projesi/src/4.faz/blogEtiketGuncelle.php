       ...
       if($c2 = OCILogon("C##b21328394", "21328394", $db2))
          {
		    ...
		    $array=oci_parse($c2,"update  blog_tag set tag_name='$title' where blog_tag_id =".$_GET['etiket_id']);		
            oci_execute($array);
			$array=oci_parse($c2,"insert into logUser values(DEFAULT,'$person_id','Blog etiketi guncelliyor.',to_date('$delivDate','dd-mm-yy hh24:mi:ss'))");		
            oci_execute($array);
				
		 }  
	  oci_close($c2);
	  header("Location:blogEtiketler.php");
   ...
   <body  style="background-color:powderblue; !important;">  
    ...
    <div id="menu">
       ...
       <?php			
		    ...
		    $array3=oci_parse($c,"select tag_name from blog_tag where  blog_tag_id =".$_GET['etiket_id']);
	        oci_execute($array3);
			$row=oci_fetch_array($array3);
       ?>  
            ...
            <div id="icerik"style="background-color:powderblue; !important;">
               <br/>
			   <span style="font-weight:bold;">Etiket Güncelleme </span>
			    <hr/>
			    <br/>
			<form method="post">
	         	<table>
	              <tr>
		           <td><span style="font-weight:bold;">Etiket İsim:</span></td>
		           <td><input type="text" name="baslik" value="<?php echo $row[0];?>" /></td>
		          </tr>
	             <tr>
